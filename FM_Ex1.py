"""
In this program we implemented of Algorithm 1  for the Fisher equation. Also,
...
# =============================================================================
# Parameters used for the example 1: Fisher equation
# =============================================================================
...
theta = 5                           # Parameter in the Fisher equation
...
def u_true(x, t, r):
    """
        Exact solution for the Fisher equation
    ...
    """
    sol = 1 / (1 + np.exp(np.sqrt(r / 6) * x - 5 / 6 * r * t)) ** 2
...
def plot_num_vs_exac_t_x(theta):
    ...
    plt.ylabel('u(t,x)')
    plt.legend()
    # plt.savefig("NumericvsTrue_ex1.eps")  # Uncomment the line for saving the figure
...
def Error_exact(X, grid_x, grid_t, theta):
    """
     ...
     ----------
     X: array
         Numerical solution in the grid (x,t)
     theta: float
          Parameter in the Fisher equation
     grid_t: float
         Temporal step size
...
def plot_error_ex_est(dx, theta, num=4):
    ...
    fig, ax = plt.subplots()
    ax.loglog(H, e_exact_max, "black", marker="v", label='Exact Error')
    ax.loglog(H, e_s_max, "r-.", marker="s", label='Numeric Error')
    ...
    plt.ylabel('Error')
    plt.legend()
    # plt.savefig("Error_ex1.eps")  # Uncomment the line for saving the figure
...
def Error_order(theta):
    ...
    plt.figure()
    plt.plot(hs, order, marker="*")
    plt.xlabel('h')
    plt.ylim(2.02,1.98)
    plt.ylabel('Order')
    # plt.savefig("Order_ex1.eps")  # Uncomment the line for saving the figure
    return order

# -------- Figures for Example 1 ---------
# To plot Figures 1a, 2ac(for example 1) and the Table 1 (column 2) uncommented the following lines,

# plot_num_vs_exac_t_x(theta)               # Fig 1a
# plot_error_ex_est(dx, theta, num=4)       # Fig 2a, example 1
# Error_order(theta)                        # Table 1 (column 2)